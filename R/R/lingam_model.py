...
def causal_prediction(
  ...
  ,prior_knowledge=None
    ...
    # Calculate all NRMSE
    nrmse_train = sqrt(mean((y_tr_trans - y_tr_pred)**2)) / (max(y_tr_trans) - min(y_tr_trans))
    if x_val_trans is not None:
      nrmse_val = sqrt(mean(sum((y_val_trans - y_val_pred)**2))) / (max(y_val_trans) - min(y_val_trans))
      nrmse_test = sqrt(mean(sum((y_test_trans - y_test_pred)**2))) / (max(y_test_trans) - min(y_test_trans))
    else:
